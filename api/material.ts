...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    try {
        ...
        if (req.method === 'GET') {
            ...
            const entriesQuery = `SELECT * FROM stock_entries ORDER BY date DESC, id DESC LIMIT $1 OFFSET $2`;
            const countQuery = `SELECT COUNT(*) FROM stock_entries`;
            const summaryQuery = `
                SELECT
            ...
            const [entriesResult, countResult, summaryResult] = await Promise.all([
                client.query(entriesQuery, [limit, offset]),
